...
void RVSSVM::Fetch() {
  if_id_register.instruction = memory_controller_.ReadWord(program_counter_);
  if_id_register.pc = program_counter_;
  UpdateProgramCounter(4);
}

void RVSSVM::Decode() {
  uint32_t current_instruction = if_id_register.instruction;
  control_unit_.SetControlSignals(current_instruction);

  // write the value of rs1, rs2 and other stuff in the id_ex_register
  id_ex_register.pc = if_id_register.pc;
  id_ex_register.instruction = if_id_register.instruction;
  id_ex_register.rs1_num = (current_instruction >> 15) & 0b11111;
  id_ex_register.rs2_num = (current_instruction >> 20) & 0b11111;
  id_ex_register.imm = ImmGenerator(current_instruction);
  id_ex_register.reg1_value = registers_.ReadGpr(id_ex_register.rs1_num);
  id_ex_register.reg2_value = registers_.ReadGpr(id_ex_register.rs2_num);
  id_ex_register.rd_num = (current_instruction >> 7) & 0b11111;
  ...
  id_ex_register.alu_op_ = control_unit_.GetAluOp();
  id_ex_register.reg_write = control_unit_.GetRegWrite();
  id_ex_register.mem_read = control_unit_.GetMemRead();
  id_ex_register.mem_write = control_unit_.GetMemWrite();
  id_ex_register.mem_to_reg = control_unit_.GetMemToReg();
  id_ex_register.alu_src = control_unit_.GetAluSrc();
  id_ex_register.branch = control_unit_.GetBranch();
  
  
}

void RVSSVM::Execute() {
  uint32_t current_instruction = id_ex_register.instruction;
  ...
  // a
  uint8_t rs1 = id_ex_register.rs1_num;
  uint8_t rs2 = id_ex_register.rs2_num;
  
  // int32_t imm = ImmGenerator(current_instruction);
  
  // a
  uint32_t imm = id_ex_register.imm;

  // uint64_t reg1_value = registers_.ReadGpr(rs1);
  // uint64_t reg2_value = registers_.ReadGpr(rs2);
  
  // a
  uint64_t reg1_value = id_ex_register.reg1_value;
  uint64_t reg2_value = id_ex_register.reg2_value;

  bool overflow = false;

  if (id_ex_register.alu_src) {
    reg2_value = static_cast<uint64_t>(static_cast<int64_t>(imm));
  }

  alu::AluOp aluOperation = control_unit_.GetAluSignal(current_instruction, id_ex_register.alu_op_);
  std::tie(execution_result_, overflow) = alu_.execute(aluOperation, reg1_value, reg2_value);


  if (id_ex_register.branch) {
  ...
  if (branch_flag_ && opcode==0b1100011) {
    ...
    UpdateProgramCounter(imm);
    // a
    ex_mem_register.branch_target_pc = program_counter_;
  }
  ...
  ex_mem_register.alu_result = execution_result_;
  ex_mem_register.branch_taken = branch_flag_;
  ex_mem_register.next_pc = next_pc_;

  // std::cout<<"hello"<<ex_mem_register.alu_result<<std::endl;

  // forwarding the current values from the intermediate registers
  ex_mem_register.instruction = id_ex_register.instruction;
  ex_mem_register.rd_num = id_ex_register.rd_num;
  ex_mem_register.reg2_value = id_ex_register.reg2_value;
  ex_mem_register.reg_write = id_ex_register.reg_write;
  ex_mem_register.mem_read = id_ex_register.mem_read;
  ex_mem_register.mem_write = id_ex_register.mem_write;
  ex_mem_register.mem_to_reg = id_ex_register.mem_to_reg;
}
...
void RVSSVM::WriteMemory() {
  uint32_t current_instruction = ex_mem_register.instruction;
  uint64_t execution_result = ex_mem_register.alu_result;
  uint8_t opcode = current_instruction & 0b1111111;
  uint8_t rs2 = (current_instruction >> 20) & 0b11111;
  uint8_t funct3 = (current_instruction >> 12) & 0b111;
  ...
  mem_wb_register.memory_read_data = memory_result_;

  // forwarding the data from ex_mem_register to mem_wb_register
  mem_wb_register.next_pc = ex_mem_register.next_pc;
  mem_wb_register.alu_result = ex_mem_register.alu_result;
  mem_wb_register.instruction = ex_mem_register.instruction;
  mem_wb_register.rd_num = ex_mem_register.rd_num;
  mem_wb_register.reg_write = ex_mem_register.reg_write;
  mem_wb_register.mem_to_reg = ex_mem_register.mem_to_reg;
}
...
void RVSSVM::WriteBack() {
  uint32_t current_instruction = mem_wb_register.instruction;
  uint8_t opcode = current_instruction & 0b1111111;
  uint8_t funct3 = (current_instruction >> 12) & 0b111;
...
void RVSSVM::Run() {
  ...
  for(int i=0;i<32;i++) std::cout<<registers_.ReadGpr(i)<<" ";
  std::cout<<std::endl;
